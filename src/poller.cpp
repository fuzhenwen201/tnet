...
namespace tnet
{
        ...
        : m_loop(loop)
    {
        ...
        if(m_fd < 0)
        {
            XERROR("epoll create error %d", errno);
        } 
    ...
    int Poller::add(int fd, int events)
    {
        ...
        if(ret < 0)
        {
            XERROR("epoll_ctl add error  %d", errno);
            return -1;
        }
    ...
    int Poller::update(int fd, int events)
    {
        ...
        if(ret < 0)
        {
            XERROR("epoll_ctl update error  %d", errno);
            return -1;    
        }
    ...
    int Poller::remove(int fd)
    {
        ...
        if(ret < 0)
        {
            XERROR("epoll_ctl remove error  %d", errno);
            return -1;    
        }
    ...
    int Poller::poll(int timeout, const std::vector<IOEvent*>& events)
    {
        ...
        if(ret < 0)
        {
            XERROR("epoll_wait error  %d", errno);
            return -1;    
        }
        ...
        for(int i = 0; i < ret; ++i)
        {
            ...
            if(got & ~want)
            {
                ...
                { 
                    XERROR("ctl error %d got:%d, want:%d, fd:%d", errno, got, want, fd);
                    continue;
                }
            }
