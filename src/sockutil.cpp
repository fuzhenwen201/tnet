...
namespace tnet
{
    ...
    int SockUtil::bindAndListen(const Address& addr)
    {
        ...
        {
            err = errno;
            XERROR("create socket error  %d", errno);
            return fd;    
        }
        ...
        do
        {
            ...
            {
                err = errno;
                XERROR("bind address %s:%d error: %d", addr.ipstr().c_str(), addr.port(), err);
                break;    
            }
            ...
            {
                err = errno;
                XERROR("listen address %s:%d error: %d", addr.ipstr().c_str(), addr.port(), err);
                break;     
            }
    ...
    uint32_t SockUtil::getHostByName(const string& host)
    {
        ...
        if(ret != 0)
        {
            XERROR("getaddrinfo error  %d", errno);
            return uint32_t(-1);
        }
    ...
    int SockUtil::bindDevice(int sockFd, const string& device)
    {
        ...
        {
            XERROR("ioctl get addr error  %d", errno);
            return -1;
        }
        ...
        {
            XERROR("bind interface error  %d", errno);
            return -1;
        } 
