...
namespace tnet
{
        ...
        , m_repeated(false)
    {
        ...
        if(m_fd < 0)
        {
            XERROR("create timer error  %d", errno);
            return;
        } 
    ...
    void Timer::start(IOLoop* loop)
    {
        ...
        if(m_running)
        {
            XWARNING("timer was started");
            return;    
        }

        XINFO("start timer %d", m_fd);
